
\documentclass[graybox,envcountchap,sectrefs]{svmono}

\usepackage{mathptmx}
\usepackage{helvet}
\usepackage{courier}
\usepackage{type1cm}
\usepackage{makeidx}   
\usepackage{graphicx}     
                            
\usepackage{multicol}     
\usepackage[bottom]{footmisc}

\usepackage{amsmath, amssymb, graphicx, graphics, float}
\usepackage{rotating}
\usepackage{sidecap}

\usepackage[colorlinks=true,citecolor=black,linkcolor=black,urlcolor=blue]{hyperref}

\newcommand{\w}{\mathbf}
\newcommand{\s}{\boldsymbol}
\newcommand{\dd}[1][n]{\left[\left[{#1}\right]\right]_{-}^{+}}

\newcommand{\delln}{\dot{\ell}^{(N)}}
\newcommand{\dell}{\dot{\ell}}
\newcommand{\elln}{\ell^{(N)}}

\newcommand{\vpv}{v_\text{\tiny{PV}}}
\newcommand{\vph}{v_\text{\tiny{PH}}}
\newcommand{\vsv}{v_\text{\tiny{SV}}}
\newcommand{\vsh}{v_\text{\tiny{SH}}}
\newcommand{\vp}{v_\text{\tiny{P}}}
\newcommand{\vs}{v_\text{\tiny{S}}}
\newcommand{\xis}{\xi_\text{\tiny{S}}}
\newcommand{\xip}{\xi_\text{\tiny{P}}}
\newcommand{\vpi}{\bar{v}_\text{\tiny{P}}}
\newcommand{\vsi}{\bar{v}_\text{\tiny{S}}}
\newcommand{\tr}{\text{tr}\,}
\newcommand{\R}{\mathbb{R}}
\newcommand{\tu}{\tilde{u}_i}
\newcommand{\sign}{\text{sign\,}}
\newcommand{\p}{\partial}

\newcommand{\bpsi}{\bar{\psi}}
\newcommand{\bphi}{\bar{\phi}}

\newcommand{\SES}{\texttt{SES3D}$\,$}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}

\title{ 	\Huge{\textbf{SES3D} version 5.0}\\[10pt] 
	\large{Documentation - \textcolor{red}{Work in progress.}}\\[100pt]
	\LARGE{Andreas Fichtner}\\[5pt] 
	\normalsize{Department of Earth Sciences, ETH Zurich, Switzerland}\\[15pt]
	\LARGE{Alexey Gokhberg}\\[5pt] 
	\normalsize{Department of Earth Sciences, ETH Zurich, Switzerland}\\[15pt]
	\LARGE{Lion Krischer}\\[5pt]
	\normalsize{Department of Earth Sciences, ETH Zurich, Switzerland}\\[90pt]
	\normalsize{14 February 2014} }

\maketitle

\frontmatter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\tableofcontents

\include{foreword}

\mainmatter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%- code description %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\include{Part_1/part_1}      
	
	\include{Part_1/01_code_structure}                  
	\include{Part_1/02_scenarios}
	\include{Part_1/03_python}                            

%- inverse problem %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\include{Part_2/part_2}          

	\include{Part_2/01_wave_equation}        
	\include{Part_2/02_attenuation}
	\include{Part_2/03_absorbing_boundaries}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\include{bibliography}

\end{document}
